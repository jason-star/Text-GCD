...
from data.talkmoves import get_talkmoves_datasets
...
from data.talkmoves import subsample_classes as subsample_dataset_talkmoves
...
sub_sample_class_funcs = {
    ...
    'talkmoves': subsample_dataset_talkmoves,
}
...
get_dataset_funcs = {
    ...
    'talkmoves': get_talkmoves_datasets,
}
...
def get_class_splits(args):
    ...
    if args.dataset_name == 'cifar10':
        ...
        args.image_size = 224
        herb_path_splits = os.path.join(osr_split_dir, '/home/zhanglu/GCD/generalized-category-discovery-main/data/ssb_splits/herbarium_19_class_splits.pkl')
        ...
        if use_ssb_splits:
            split_path = os.path.join(osr_split_dir, '/home/zhanglu/GCD/generalized-category-discovery-main/data/ssb_splits/scars_osr_splits.pkl')
        ...
        args.image_size = 224
        if use_ssb_splits:
            split_path = os.path.join(osr_split_dir, '/home/zhanglu/GCD/generalized-category-discovery-main/data/ssb_splits/aircraft_osr_splits.pkl')
            with open(split_path, 'rb') as handle:
                class_splits = pickle.load(handle)
        ...
        if use_ssb_splits:
            split_path = os.path.join(osr_split_dir, '/home/zhanglu/GCD/generalized-category-discovery-main/data/ssb_splits/cub_osr_splits.pkl')
            with open(split_path, 'rb') as handle:
                class_splits = pickle.load(handle)
    ...
    elif args.dataset_name == 'talkmoves':
        
        # TalkMoves数据集有7个类别（0-6）
        # 设置0-3为已知类别，4-6为未知类别
        args.image_size = None  # 文本数据集不需要图像大小
